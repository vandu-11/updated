...
function DecemberCalendar() {
  ...
  const eventsData = [
    {
      start: new Date(2023, 11, 5, 10, 0), // December is represented by 11 (0-based index)
  ...
  const formattedEventsData = eventsData.map((event) => ({
    ...event,
    start: moment(event.start).format('YYYY-MM-DD hh:mm A'), // e.g., "2023-12-05 10:00 AM"
    end: moment(event.end).format('YYYY-MM-DD hh:mm A'),     // e.g., "2023-12-05 11:00 AM"
  }));
